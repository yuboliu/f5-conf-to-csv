...
conf_files = [filename for filename in os.listdir() if filename.endswith('.conf')]
file_names_without_extension = [os.path.splitext(filename)[0] for filename in conf_files]
print("扩展名为conf的文件名列表:", conf_files)
print("去掉扩展名后的文件名列表:", file_names_without_extension)

directories = ['DB', 'Output']
# 创建目录
...
# Initialize Database
for conf_file, file_name in zip(conf_files, file_names_without_extension):
    print(conf_file, file_name)
    F5_db = '{}.sqlite3'.format(file_name)
    conn = sqlite3.connect('DB/{}'.format(F5_db))
    ...
    File = open("{}".format(conf_file), "r", encoding='utf-8')
    ...
    conn.commit()
    directory = 'Output/{}'.format(file_name)
    if not os.path.exists(directory):
        os.makedirs(directory)
    # Write Tables to CSV
    write_csv(conn, "SELECT * FROM Nodes", "Output/{}/f5-nodes.csv".format(file_name))
    write_csv(conn, "SELECT * FROM Pools", "Output/{}/f5-pools.csv".format(file_name))
    write_csv(conn, "SELECT * FROM VirtualAddresses", "Output/{}/f5-virtualaddresses.csv".format(file_name))
    write_csv(conn, "SELECT * FROM VirtualServers", "Output/{}/f5-virtualservers.csv".format(file_name))
    write_csv(conn, "SELECT * FROM Monitors", "Output/{}/f5-monitors.csv".format(file_name))
    write_csv(conn, "SELECT * FROM Profiles", "Output/{}/f5-profiles.csv".format(file_name))
...
for subfolder in subfolders:
    excel_name = os.path.basename(subfolder) + ".xlsx"
    excel_path = os.path.join(subfolder, excel_name)

    # 获取子文件夹中的所有CSV文件
    csv_files = [filename for filename in os.listdir(subfolder) if filename.endswith('.csv')]

    # 创建一个Excel writer对象
    writer = pd.ExcelWriter(excel_path, engine='xlsxwriter')

    # 遍历每个CSV文件，将其写入Excel工作簿
    ...
    # 保存Excel工作簿
    writer.close()
    print(f"Excel工作簿'{excel_name}'已创建。")
